...
from backend.models.traffic_schemas import ApproachEnum, ApproachTrafficState, MovementStateEnum, TrafficLevelEnum
...
class TrafficMetricsCalculator:
        ...
        for v in vehicles:
            ...
            if v.previous_center is not None:
                if intersect(v.previous_center, v.center, p1, p2):
                    ...
                    # Record directional crossing
                    v_dir = getattr(v, "direction", None)
                    if v_dir in (MovementStateEnum.OUTGOING, MovementStateEnum.STOPPED_OUTGOING):
    ...
    def calculate_metrics(
        ...
        """
        # 1. Filter active (non-parked) vehicles for active traffic metrics
        active_vehicles = [
            v for v in vehicles
            if not getattr(v, "is_parked", False) and getattr(v, "direction", None) != MovementStateEnum.PARKED
        ]
        active_count = len(active_vehicles)

        # 2. Vehicle count by class for active vehicles
        ...
        # 5. Estimated Queue Length (vehicles with low speed / stationary for >= 5 frames)
        estimated_queue = sum(1 for v in active_vehicles if v.stationary_frames >= 5 or v.speed_px < settings.QUEUE_SPEED_THRESHOLD)

        # 6. Directional vehicle counts
        incoming_count = sum(
            1 for v in active_vehicles
            if getattr(v, "direction", None) in (MovementStateEnum.INCOMING, MovementStateEnum.STOPPED_INCOMING)
        )
        outgoing_count = sum(
            1 for v in active_vehicles
            if getattr(v, "direction", None) in (MovementStateEnum.OUTGOING, MovementStateEnum.STOPPED_OUTGOING)
        )
        stopped_incoming_count = sum(
            1 for v in active_vehicles
            if getattr(v, "direction", None) == MovementStateEnum.STOPPED_INCOMING
        )
        stopped_outgoing_count = sum(
            1 for v in active_vehicles
            if getattr(v, "direction", None) == MovementStateEnum.STOPPED_OUTGOING
        )
        parked_count = sum(
            1 for v in vehicles
            if getattr(v, "is_parked", False) or getattr(v, "direction", None) == MovementStateEnum.PARKED
        )
        unknown_direction_count = sum(
            1 for v in active_vehicles
            if getattr(v, "direction", None) == MovementStateEnum.UNKNOWN or getattr(v, "direction", None) is None
        )

        # 7. Traffic Level classification
