...
def run_test():
    print("=" * 60, flush=True)
    print("RUNNING CUDA & IMGSZ=960 SMALL INFERENCE TEST", flush=True)
    print("=" * 60, flush=True)
    ...
    output_video = PROJECT_ROOT / "data" / "annotated" / "my_traffic_test_960.mp4"
    ...
    # Process small test (first 90 frames)
    ...
    try:
        while cap.isOpened() and frame_idx < MAX_TEST_FRAMES:
            ...
            # Run tracking with imgsz=960, device=0
    ...
    print("\n" + "=" * 60, flush=True)
    print("SMALL TEST RESULTS (imgsz=960, device=0):", flush=True)
    print("=" * 60, flush=True)
    print(f"• Frames Tested:          {frame_idx}", flush=True)
    print(f"• Total Processing Time:  {elapsed:.2f} seconds ({frame_idx/elapsed:.1f} FPS)", flush=True)
